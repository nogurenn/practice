...
import (
	...
	"github.com/nogurenn/practice-go/leetcode"
...
func TestTwoSum(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.subtest, func(t *testing.T) {
			got := leetcode.TwoSum(tc.nums, tc.target)
			assert.Equal(t, tc.want, got)
		})
	}
}
...
func TestIsAnagram(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.subtest, func(t *testing.T) {
			got := leetcode.IsAnagram(tc.s, tc.t)
			assert.Equal(t, tc.want, got)
		})
	}
}
...
func TestLengthOfLongestSubstring(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.subtest, func(t *testing.T) {
			got := leetcode.LengthOfLongestSubstring(tc.s)
			assert.Equal(t, tc.want, got)
		})
	}
}

type CanPermutePalindromeTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		s       string
...
func (suite *CanPermutePalindromeTestSuite) SetupTest() {
...
func (suite *CanPermutePalindromeTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(string) bool
	}{
		{
			name: "Hashmap",
			fn:   leetcode.CanPermutePalindromeHashmap,
		},
		{
			name: "Array",
			fn:   leetcode.CanPermutePalindromeArray,
		},
		{
			name: "Single Pass",
			fn:   leetcode.CanPermutePalindromeSinglePass,
		},
		{
			name: "Set", // via golang-set/v2
			fn:   leetcode.CanPermutePalindromeSet,
		},
	}

	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				got := approach.fn(tc.s)
...
func TestCanPermutePalindromeTestSuite(t *testing.T) {
	suite.Run(t, new(CanPermutePalindromeTestSuite))
}

type ClimbStairsTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		n       int
...
func (suite *ClimbStairsTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		n       int
		want    int
...
func (suite *ClimbStairsTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(int) int
	}{
		{
			name: "Recursive",
			fn:   leetcode.ClimbStairsRecursive,
		},
		{
			name: "Dynamic Programming",
			fn:   leetcode.ClimbStairsDynamicProgramming,
		},
		{
			name: "Fibonacci",
			fn:   leetcode.ClimbStairsFibonacci,
		},
	}
...
func TestClimbStairsTestSuite(t *testing.T) {
	suite.Run(t, new(ClimbStairsTestSuite))
}

type SubsetsTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		nums    []int
...
func (suite *SubsetsTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		nums    []int
		want    [][]int
...
func (suite *SubsetsTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func([]int) [][]int
	}{
		{
			name: "Iterative Cascade",
			fn:   leetcode.SubsetsIterativeCascade,
		},
		{
			name: "Backtracking",
			fn:   leetcode.SubsetsBacktracking,
		},
		{
			name: "Bitmask (lexicographic, binary sorted)",
			fn:   leetcode.SubsetsBitmask,
		},
	}
	...
	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				got := approach.fn(tc.nums)
				suite.ElementsMatch(tc.want, got)
			})
		}
	}
}

func TestSubsetsTestSuite(t *testing.T) {
	suite.Run(t, new(SubsetsTestSuite))
}

type GenerateParenthesisTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		n       int
...
func (suite *GenerateParenthesisTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		n       int
		want    []string
...
func (suite *GenerateParenthesisTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(int) []string
	}{
		{
			name: "Brute Force (Queue-based)",
			fn:   leetcode.GenerateParenthesisBruteForce,
		},
		{
			name: "Backtracking (Recursive)",
			fn:   leetcode.GenerateParenthesisBacktracking,
		},
		{
			name: "Bitmasking",
			fn:   leetcode.GenerateParenthesisBitmasking,
		},
	}
...
func TestGenerateParenthesisTestSuite(t *testing.T) {
	suite.Run(t, new(GenerateParenthesisTestSuite))
}

type IntersectionOfTwoLinkedListsTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		headA   *leetcode.ListNode
		headB   *leetcode.ListNode
		want    *leetcode.ListNode
	}
}

func (suite *IntersectionOfTwoLinkedListsTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		headA   *leetcode.ListNode
		headB   *leetcode.ListNode
	...
	// headA: 4 -> 1 -> 8 -> 4 -> 5
	// headB: 5 -> 0 -> 1 -> 8 -> 4 -> 5
	// Ensure that the intersection is at node with value 8
	// Make a generator loop for the linked lists
	values := []struct {
		headA []int
		headB []int
		// index of headA where the intersection is
		// If it is negative, then there is no intersection (< 0)
		intersection int
	}{
		{
			headA:        []int{4, 1, 8, 4, 5},
	...
	for i, v := range values {
		headA := leetcode.NewListNodeFromInts(v.headA...)
		headB := leetcode.NewListNodeFromInts(v.headB...)

		// Find the intersection node
		var intersectionNode *leetcode.ListNode
		if v.intersection >= 0 {
			intersectionNode = headA
			for j := 0; j < v.intersection; j++ {
				intersectionNode = intersectionNode.Next
			}
		}

		// Link the intersection node to the rest of headB
		...
		suite.testCases = append(suite.testCases, struct {
			...
			subtest: fmt.Sprintf("Example %d", i+1),
			headA:   headA,
			headB:   headB,
...
func (suite *IntersectionOfTwoLinkedListsTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(*leetcode.ListNode, *leetcode.ListNode) *leetcode.ListNode
	}{
		{
			name: "Hashmap",
			fn:   leetcode.GetIntersectionNodeHashmap,
		},
		{
			name: "Two Pointers",
			fn:   leetcode.GetIntersectionNodeTwoPointers,
		},
	}
	...
	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				got := approach.fn(tc.headA, tc.headB)
				suite.Equal(tc.want, got)
			})
		}
	}
...
func TestIntersectionOfTwoLinkedListsTestSuite(t *testing.T) {
	suite.Run(t, new(IntersectionOfTwoLinkedListsTestSuite))
}

// Generate the test suite for the leetcode.IsHappy approaches
...
func (suite *IsHappyTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		n       int
		want    bool
...
func (suite *IsHappyTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(int) bool
	}{
		{
			name: "HashMap",
			fn:   leetcode.IsHappyHashMap,
		},
		{
			name: "Two Pointers",
			fn:   leetcode.IsHappyTwoPointers,
		},
	}
...
// Generate the test suite for the leetcode.MergeSortedArrays approaches
type MergeSortedArraysTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		nums1   []int
...
func (suite *MergeSortedArraysTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		nums1   []int
		m       int
...
func (suite *MergeSortedArraysTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		...
		{
			name: "Three Pointers",
			fn:   leetcode.MergeSortedArraysThreePointersBeginning,
		},
		{
			name: "Three Pointers (starting from end)",
			fn:   leetcode.MergeSortedArraysThreePointersEnd,
		},
	}
	...
	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				// Copy the original nums1 to avoid modifying it
				nums1 := make([]int, len(tc.nums1))
				copy(nums1, tc.nums1)

				approach.fn(nums1, tc.m, tc.nums2, tc.n)
				suite.Equal(tc.want, nums1)
			})
		}
	}
}

func TestMergeSortedArraysTestSuite(t *testing.T) {
	suite.Run(t, new(MergeSortedArraysTestSuite))
}

type SearchInRotatedSortedArrayTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		nums    []int
...
func (suite *SearchInRotatedSortedArrayTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		nums    []int
		target  int
...
func (suite *SearchInRotatedSortedArrayTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func([]int, int) int
	}{
		{
			name: "Pivot Index + Binary Search",
			fn:   leetcode.SearchInRotatedSortedArrayPivotIndex,
		},
		{
			name: "One Binary Search",
			fn:   leetcode.SearchInRotatedSortedArrayOneBinarySearch,
		},
	}
	...
	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				got := approach.fn(tc.nums, tc.target)
				suite.Equal(tc.want, got)
			})
		}
	}
...
func TestSearchInRotatedSortedArrayTestSuite(t *testing.T) {
	suite.Run(t, new(SearchInRotatedSortedArrayTestSuite))
}

type FindMedianOfTwoSortedArraysTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		nums1   []int
...
func (suite *FindMedianOfTwoSortedArraysTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		nums1   []int
		nums2   []int
...
func (suite *FindMedianOfTwoSortedArraysTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		...
		{
			name: "Merge and Sort",
			fn:   leetcode.FindMedianSortedArraysMergeSort,
		},
		// {
		// 	name: "Binary Search",
		// 	fn:   leetcode.FindMedianSortedArraysBinarySearchRecursive,
		// },
	}
	...
	for _, approach := range approaches {
		for _, tc := range suite.testCases {
			suite.Run(approach.name+"/"+tc.subtest, func() {
				got := approach.fn(tc.nums1, tc.nums2)
				suite.Equal(tc.want, got)
			})
		}
	}
...
func TestFindMedianOfTwoSortedArraysTestSuite(t *testing.T) {
	suite.Run(t, new(FindMedianOfTwoSortedArraysTestSuite))
}

// Generate the test suite for leetcode.MaximumDepthOfBinaryTree
type MaximumDepthOfBinaryTreeTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
...
func (suite *MaximumDepthOfBinaryTreeTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		root    *leetcode.TreeNode
		want    int
		...
		{
			subtest: "Example 1",
			root:    leetcode.NewBinaryTreeFromSlice([]interface{}{3, 9, 20, nil, nil, 15, 7}),
			want:    3,
		},
		{
			subtest: "Example 2",
			root:    leetcode.NewBinaryTreeFromSlice([]interface{}{1, nil, 2}),
			want:    2,
		},
		{
			subtest: "Example 3",
...
func (suite *MaximumDepthOfBinaryTreeTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(*leetcode.TreeNode) int
	}{
		{
			name: "Depth First Search (Recursive)",
			fn:   leetcode.MaximumDepthOfBinaryTreeDFSRecursive,
		},
		// {
		// 	name: "Breadth First Search",
		// 	fn:   leetcode.MaximumDepthOfBinaryTreeBFS,
		// },
	}
...
func TestMaximumDepthOfBinaryTreeTestSuite(t *testing.T) {
	suite.Run(t, new(MaximumDepthOfBinaryTreeTestSuite))
}

// Generate the test suite for leetcode.ValidateBinarySearchTree

type ValidateBinarySearchTreeTestSuite struct {
	suite.Suite
	testCases []struct {
		subtest string
		root    *leetcode.TreeNode
...
func (suite *ValidateBinarySearchTreeTestSuite) SetupTest() {
	suite.testCases = []struct {
		subtest string
		root    *leetcode.TreeNode
		want    bool
		...
		{
			subtest: "Example 1",
			root:    leetcode.NewBinaryTreeFromSlice([]interface{}{2, 1, 3}),
			want:    true,
		},
		{
			subtest: "Example 2",
			root:    leetcode.NewBinaryTreeFromSlice([]interface{}{5, 1, 4, nil, nil, 3, 6}),
			want:    false,
		},
		{
			subtest: "Example 3",
...
func (suite *ValidateBinarySearchTreeTestSuite) TestApproaches() {
	approaches := []struct {
		name string
		fn   func(*leetcode.TreeNode) bool
	}{
		{
			name: "Depth First Search (Recursive)",
			fn:   leetcode.IsValidBSTRecursiveTraversalWithValidRange,
		},
	}
